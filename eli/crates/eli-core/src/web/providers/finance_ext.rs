...
async fn search_congress_trades(query: &str) -> Result<Vec<WebHit>> {
    ...
    let hits = trades
        ...
        .map(|t| WebHit {
            title: format!(
                "{} trade: {} by {}",
                t.type_.as_deref().unwrap_or_default(),
                t.ticker.as_deref().unwrap_or_default(),
                t.representative.as_deref().unwrap_or_default()
            ),
            url: "https://housestockwatcher.com/".to_string(),
            snippet: format!(
                "Amount: {}, Date: {}",
                t.amount.as_deref().unwrap_or_default(),
                t.transaction_date.as_deref().unwrap_or_default()
            ),
            source: "HouseStockWatcher".to_string(),
            score: 0.95,
            published: t.transaction_date.as_ref().and_then(|d| {
