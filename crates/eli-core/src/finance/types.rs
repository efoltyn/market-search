...
pub fn parse_as_of(raw: &str) -> Result<DateTime<Utc>> {
    ...
    let date = chrono::NaiveDate::parse_from_str(s, "%Y-%m-%d").map_err(|_| {
        Error::InvalidInput(format!("invalid as-of '{raw}' (use YYYY-MM-DD or RFC3339)"))
    })?;
...
pub(crate) fn default_risk_free_rate_annual() -> f64 {
    0.02
}
...
pub(crate) fn default_odds_field_semantics() -> OddsFieldSemantics {
    OddsFieldSemantics {
